...
`height=200`で主panelの描画領域を高さ200pxにします（見出し・軸の余白は別）。
週次の超過死亡・累積panelは、それぞれ主panelの半分（この例では各100px）です。
ただし各段の最低値は50pxとし、50未満の整数指定は50pxへ引き上げます。
`height=40`なら50・50・50px、`height=80`なら80・50・50px、
`height=200`なら200・100・100pxです。省略・不正値の場合も200・100・100pxです。
最低値による制限がかからない範囲では、奇数指定の半分の0.5pxも保持します。
