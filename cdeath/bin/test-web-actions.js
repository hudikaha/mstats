...
const tests = [
  ...
  },
  {
    ...
    noNavigation: true,
    expect: `document.querySelector('.model-option[value="poisson"]')?.checked && new URL(location.href).searchParams.get('family') === 'poisson' && new URL(location.href).searchParams.get('interval') === 'sim' && !new URL(location.href).searchParams.has('chart_model') && getComputedStyle(document.querySelector('#simulation-interval-control')).display !== 'none'`
  },
  {
  ...
  },
  {
    ...
    action: `(() => { const url = new URL(location.href); url.searchParams.set('start_year', '2019'); history.replaceState(null, '', url); const slider = document.querySelector('#start-year-slider'); slider.value = '2020'; slider.dispatchEvent(new Event('input', {bubbles:true})); document.querySelector('#zero-base-checkbox').click(); const deficit = document.querySelector('#deficit-checkbox'); if (deficit.checked) deficit.click(); deficit.click(); document.querySelector('#covid-overlay-checkbox').click(); document.querySelector('#vaxx-overlay-checkbox').click(); document.querySelector('.mortyear-form').requestSubmit(); return true; })()`,
    expect: `(() => new URL(location.href).searchParams.get('from') === '2020' && ['zero','deficit','covid','vaxx'].every(name => new URL(location.href).searchParams.get(name) === '1') && ['start_year','zero_base','include_deficit','covid_overlay','vaxx_overlay'].every(name => !new URL(location.href).searchParams.has(name)) && document.querySelector('#start-year-hidden')?.value === '2020' && document.querySelector('#start-year-slider')?.max === '2020' && document.querySelector('#start-year-slider')?.value === '2020' && document.querySelector('#zero-base-checkbox')?.checked && document.querySelector('#deficit-checkbox')?.checked && document.querySelector('#covid-overlay-checkbox')?.checked && document.querySelector('#vaxx-overlay-checkbox')?.checked && weeklyValues.some(item => item.excess < 0) && window.mortyearView?.signal('zero_base') === true && window.mortyearView?.signal('include_deficit') === true && window.mortyearView?.signal('show_covid_overlay') === true && window.mortyearView?.signal('show_vaxx_overlay') === true)()`
  }
