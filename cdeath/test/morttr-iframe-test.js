...
async function inspect(url, embedded) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 160; attempt++) {
      const reply = await send('Runtime.evaluate', {returnByValue:true, expression:`(() => {
        ...
        return {graphs:doc.querySelectorAll('#mortyear-vis canvas, #mortyear-vis svg').length,
...
(async () => {
  ...
  for (const period of ['calendar', 'weekly']) {
    let baseline;
    for (const value of ['', '1', 'on', 'true', 'ON', '0', 'off', 'false', 'other']) {
      const query = `l=ja&c=jpn&metric=deaths&period=${period}&i=${value}`;
      ...
      html = output.split(/\r?\n\r?\n/).slice(1).join('\n\n');
      const values = await inspect(`http://127.0.0.1:${server.address().port}/host?${query}`, ['1', 'on', 'true'].includes(value.toLowerCase()));
      ...
      console.log(`OK ${path.basename(target)} ${period} i=${value || '(empty)'}`);
    }
  }
