...
if Cities['cze'][:sel]
    ...
    # チェコ公式dataはそれ自体が公開され再現可能なので、無接頭辞の通常系列を表示する。
    # Czech official data is already public and reproducible, so use the regular unprefixed series.
    Sources.each_value { |value| value[:sel] = nil }
    Sources['anon'][:ja] = 'チェコ政府公式個票から計算'
    Sources['anon'][:en] = 'Calculated from Czech government official records'
