...
diagnostic_jp2015std = cgi['asr_standard'] == 'jp2015std'
...
panel_label = lambda do |loc, cause|
  ...
  parts << cause_name
  if diagnostic_jp2015std && selected_locations == ['JPN'] && selected_metric == 'asr'
    parts << ($l == :ja ? '（jp2015std再計算）' : '(recalculated jp2015std)')
  end
...
annual_by_age = { selected_ages => begin
  annual = if selected_period != 'calendar' && selected_metric == 'asr'
             ...
             standard_groups = if diagnostic_jp2015std && selected_locations == ['JPN']
                                 JPN_2015_STANDARD_GROUPS
                               end
             stratified_asr_rows(annual_records_all, selected_locations, selected_sex,
                                 selected_causes, standard_groups: standard_groups)
           elsif selected_metric != 'std_deaths'
